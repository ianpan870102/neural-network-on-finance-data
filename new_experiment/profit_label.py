...
def trading_label():
  ...
  for single_date in date_range(start_date, end_date):
    ...
    for i in df.index:
      ...
      df.loc[i, 'horizon avg'] = df.loc[i + 1:i +
                                        predict_horizon]['close'].sum() / float(predict_horizon)
    ...
    plt.suptitle('date={}, k={}, threshold={}'.format(single_date, predict_horizon,
                                                      label_threshold))
    ...
    close_price = [(float(i) - min(close_price)) / (max(close_price) - min(close_price))
    ...
    plt.plot(close_price)
    plt.title('#lables={}, max_period={}'.format(len(tans), tans_stats[-1][2]))
    plt.savefig('date={}_k={}_threshold={}.png'.format(single_date, predict_horizon,
