...
def get_model_data(df, input_size, feature_num, pred_k):
  ...
  for i in range(input_shape):  #range = 0~31837
    X.append(data[
      i:i + input_size,
      0:feature_num])  # [every 100 events from 31937 rows, take the first 40 columns as features]
    Y.append(data[i + input_size - 1,
                  -1:])  # [from 99~31936 rows, take the last 5 columns as labels]
...
def main():
  ...
  data_list = [[
    a, b, c, d, e, f, g, h
  ] for a, b, c, d, e, f, g, h in product(input_list, pred_list, feature_list, threshold_list,
                                          lstm_list, lr_list, epsilon_list, regularizer_list)]

  hyper_df = pd.DataFrame(np.array(data_list),
                          columns=[
                            'input', 'k', 'feature_num', 'label_threshold', 'lstm_units',
                            'learning_rate', 'epsilon', 'regularizer'
                          ])

  #hyper_df['data_set'] = hyper_df.index+1

  hyper_df = hyper_df.sort_values(by=[
    'input', 'k', 'feature_num', 'label_threshold', 'lstm_units', 'learning_rate', 'epsilon',
    'regularizer'
  ])
  ...
  hyper_df = hyper_df[[
    'task_id', 'input', 'k', 'feature_num', 'label_threshold', 'lstm_units', 'learning_rate',
    'epsilon', 'regularizer', 'data_set'
  ]]
